#!/usr/bin/env bash
# Fast sanity check before overnight batch.
# Runs each unique config for ~20 images (first weather only, no continual),
# writes a small log, and grep-checks that eval finished without crashing.
set -euo pipefail
GPU=${1:-0}

CONFIGS=(
  "detectron2/configs/Cityscapes/ctcmt_v2_seed0_pfn_R_50_ACDC.yaml"
  "detectron2/configs/Cityscapes/ctcmt_mtl_ctcl_no_v2_seed0_pfn_R_50_ACDC.yaml"
)
...
for CFG in "${CONFIGS[@]}"; do
  ...
  docker run --rm --gpus "\"device=${GPU}\"" --shm-size=8g \
    ...
    amrod:latest bash -c "
      ...
      # Fog only + evaluate to keep it under ~2min per config.
      timeout 240 python detectron2/tools/train_net.py \
        --config-file ${CFG} \
        --eval-only --num-gpus 1 \
        DATASETS.TEST '(\"acdc_fog_mtl\",)' \
        OUTPUT_DIR ${OUT} 2>&1 | tail -80
    " || { echo "SMOKE_FAIL: ${NAME}"; exit 1; }
  echo "=== SMOKE OK: ${NAME} ==="
done
