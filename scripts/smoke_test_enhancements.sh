#!/usr/bin/env bash
# Smoke test for the E1..E6 enhancement configs. Runs the kitchen-sink (E6)
# on fog only so we exercise all 5 feature flags in the same forward pass.
# Streams live to stdout AND tees to /tmp for reference.
set -euo pipefail
GPU=${1:-0}

CONFIGS=(
  "detectron2/configs/Cityscapes/ctcmt_e6_all_seed0_pfn_R_50_ACDC.yaml"
)
